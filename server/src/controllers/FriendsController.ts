...
class FriendController {
 ...
 // we want to create an end point to send a friend object to sendFriendRequest
 makeFriendObject = async (
   request: Request,
   response: Response
 ): Promise<Response> => {
   let fRequest = new Friend();
   try {
     const { requesterid, addresseeid } = request.params;
     fRequest.requesterid = parseInt(requesterid);
     fRequest.addresseeid = parseInt(addresseeid);
     return response.status(200).json(fRequest);
   } catch (err) {
