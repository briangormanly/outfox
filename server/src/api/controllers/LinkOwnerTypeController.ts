...
class LinkOwnerTypeController {
     ...
     */
    getAllLinkOwnerType = async (request: Request, response: Response) => {
        try {
     ...
     * @param response HTTP browser response
     */
    createLinkOwnerType = async (request: Request, response: Response) => {
        try {
     ...
     * @param response HTTP browser response
     */
    getLinkOwnerType = async (request: Request, response: Response) => {
        try {
            ...
            if (user) {
                ...
                response.status(404).send('LinkOwnerType with the specified ID does not exist');
            }
        } catch (error) {
            response.status(500).send(error.message);
        }
