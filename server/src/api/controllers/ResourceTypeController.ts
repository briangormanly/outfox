...
class ResourceTypeController {
  ...
  getResourceType = async (
    ...
    try {
      ...
      if (resourceType) {
        ...
        response
          .status(404)
          .send("Resource Type with the specified ID does not exist");
      }
    } catch (error) {
      response.status(500).send(error.message);
    }
  ...
  updateResourceType = async (
    ...
    try {
      ...
      if (updated) {
        const updatedResourceType = await ResourceType.findOne({
          where: { id: id },
        }); // Grab the update resource type
        response.status(200).json({ resourceType: updatedResourceType }); // Return the updated resource type
      } else {
        response
          .status(404)
          .send("Resource Type with the specified ID does not exist"); // Resource type does not exist
      }
    } catch (error) {
      response.status(500).send(error.message);
    }
